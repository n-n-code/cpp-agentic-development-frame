...
collect_tracked_binary_artifacts() {
    if command -v git >/dev/null 2>&1 && git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
        git ls-files | grep -E '(^|/).+\.(bin|gguf|zip|tar|tar\.gz|tar\.xz|tar\.zst)$' || true
        return
    fi
    ...
    find . \
        ...
        -name 'CMakeFiles' -prune -o \
        -name '*_autogen' -prune -o \
        \( -type f -name '*.bin' -o -type f -name '*.gguf' -o -type f -name '*.zip' -o -type f -name '*.tar' -o -type f -name '*.tar.gz' -o -type f -name '*.tar.xz' -o -type f -name '*.tar.zst' \) -print \
        | sed 's#^\./##' || true
}
...
mapfile -t markdown_files < <(find . -path './.git' -prune -o -type f -name '*.md' -print)
...
generated_artifact_matches="$({ collect_generated_artifacts || true; } | sed 's#^\./##' | awk -F/ '{ root = $1; if (!(root in seen)) { seen[root] = 1; print "./" root } }' | sort)"
...
print_violation "repository must not track binary release artifacts" "$tracked_binary_artifact_matches"
