...
run_case() {
    ...
    if [[ "$actual_result" != "$expected_result" ]]; then
        printf 'Fixture %s expected %s but got %s\n' "$fixture_name" "$expected_result" "$actual_result" >&2
        exit 1
    fi
}
...
run_case_with_policy_override() {
    ...
    if [[ "$actual_result" != "$expected_result" ]]; then
        printf 'Policy override fixture %s expected %s but got %s\n' \
            "$fixture_name" "$expected_result" "$actual_result" >&2
        exit 1
    fi
}
...
run_case missing_plan_for_substantive_change fail $'src/projectcore.cpp'
...
run_case_with_policy_override valid_contract fail $'lib/projectcore.cpp'
